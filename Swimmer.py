...
class Swimmer_env_pert(ENV_Wrapper.ENV_Adversarial_wrapper):
    ...
    def perturbate_env(self, o_act):
        ...
        force = np.array([o_act[0], o_act[1], o_act[2], 0.0, 0.0, 0.0])
        body_id = self.ids['torso']
        self.mj_data.xfrc_applied[body_id] = force
...
def main(render = True, train = False, alg = 'RARL', pm_pert = 0.0, frict = 1.0, model_to_load = '', heatmap = False):
    ...
    if alg in ['PPO', 'RARL_PPO']:
        ...
        player = net.Swimmer_NN_PPO()
        opponent = net.Swimmer_NN_PPO(n_outputs=3) # 2 output for X, Y forces on torso
    
    if alg in ['SAC', 'RARL_SAC']:
        
        player = {
            'policy': net.PolicyNetwork_SAC(n_inputs = 8, n_outputs=2, hidden = 64),
            'Q1_target': net.SoftQNetwork_SAC(n_inputs = 8 + 2, hidden = 64),
            'Q2_target': net.SoftQNetwork_SAC(n_inputs = 8 + 2, hidden = 64),
            'Q1'    : net.SoftQNetwork_SAC(n_inputs = 8 + 2, hidden = 64),
            'Q2'    : net.SoftQNetwork_SAC(n_inputs = 8 + 2, hidden = 64)
        }
        opponent = {
            'policy': net.PolicyNetwork_SAC(n_inputs = 8, n_outputs=3, hidden = 64),
            'Q1_target': net.SoftQNetwork_SAC(n_inputs = 8 + 3, hidden = 64),
            'Q2_target': net.SoftQNetwork_SAC(n_inputs = 8 + 3, hidden = 64),
            'Q1'    : net.SoftQNetwork_SAC(n_inputs = 8 + 3, hidden = 64),
            'Q2'    : net.SoftQNetwork_SAC(n_inputs = 8 + 3, hidden = 64)
        }
    ...
    if alg == 'RARL_PPO':
        rarl_ppo = PPO.RARL_PPO(player, opponent, env, print_flag=False, lr_player=1e-3, name=model_to_load)
        ...
        ppo = PPO.PPO(player, env, print_flag=False, lr=1e-3, name=model_to_load)
        if train: ppo.train(episodes=500, mini_bach=128, max_steps_rollouts=2048, continue_prev_train=False)
        ppo.load()

    if alg == 'RARL_SAC':
        rarl_sac = SAC_RARL.RARL_SAC(player, opponent, env, print_flag=False,log_alpha = 1.60944, lr_Q=3e-4, lr_pi=1e-3, name=model_to_load)
        ...
        sac = SAC_RARL.SAC(player['Q1_target'], player['Q2_target'], player['Q1'], player ['Q2'], player['policy'], env, print_flag=False, log_alpha = 1.60944, lr_Q=3e-4, lr_pi=1e-3, name= model_to_load)
        if train: sac.train(episodes=501, epoch=1, mini_batch=128, max_steps_rollouts=1024, continue_prev_train=False)
        sac.load()
    ...
    if not heatmap:
        file = f'Files/Swimmer/{alg}_{perturbation}{new_mass:0.4f}_{new_friction:0.4f}_{token}.csv'
    else:
        file = f'Files/Swimmer/heatmap/{alg}_{perturbation}{new_mass:0.4f}_{new_friction:0.4f}_{token}_heatmap.csv'
...
if __name__ == '__main__':
    #main(render=False, train=True, alg = 'PPO', model_to_load='Models/Swimmer_models/Ideal_models/Swimmer_1') # train with PPO
    #main(render=False, train=True, alg = 'RARL_PPO', model_to_load='Models/Swimmer_models/Adversarial_models/Swimmer_adversarial_1') # train with RARL
 #   main(render=False, train=True, pm_pert = 1, alg = 'PPO') # test PPO
 #   main(render=False, train=True, pm_pert = 1, alg = 'RARL_PPO') # test RARL PPO
  #  main(render=False, train=True, pm_pert = 1, alg = 'SAC', model_to_load='Models/Swimmer_models/Ideal_models/Swimmer_SAC_model') # test SAC
    #main(render=False, train=True, pm_pert = 1, alg = 'RARL_SAC', model_to_load='Models/Swimmer_models/Adversarial_models/Swimmer_adversarial_SAC_model') # test RARL SAC
    
    for path, alg in zip(['Ideal_models/Swimmer_SAC_model','Adversarial_models/Swimmer_adversarial_SAC_model'], ['SAC','RARL_SAC']): #'Ideal_models/Swimmer_1',
        for pert in [ -0.7, -0.5, -0.3, -0.1, 0.0 ,0.2, 0.5, 0.7, 0.9, 1]: #  
            for frict in [0.1, 0.4, 0.8, 1.0, 1.3, 1.7, 2.0, 2.2, 2.5]:
                main(render=False, train=False, pm_pert = pert, frict=frict, alg = alg, model_to_load = f'Models/Swimmer_models/' + path, heatmap = True) # test RARL_PPO
