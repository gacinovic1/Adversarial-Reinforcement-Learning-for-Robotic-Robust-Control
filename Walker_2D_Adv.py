...
class Walker_env_pert(ENV_Wrapper.ENV_Adversarial_wrapper):
    ...
    def step(self, action, action_adv = None):
        ...
        # make the opponent perturbing environment
        if action_adv != None:
...
def main(render = True, train = False, alg = 'RARL', pm_pert = 0, frict = 1.0, model_to_load = '', heatmap = False, perturbation = "Mass"):
    ...
    if alg == 'RARL_SAC':
        rarl_sac = SAC_RARL.RARL_SAC(player, opponent, env, print_flag=False, lr_Q=3e-4, lr_pi=e-4, name='Models/Walker_models/Adversarial_models/Walker_2D_Adversarial_SAC_model')
        ...
        sac = SAC_RARL.SAC(player['Q1_target'], player['Q2_target'], player['Q1'], player ['Q2'], player['policy'], env, print_flag=False, lr_Q=3e-4, lr_pi=1e-3, name='Models/Walker_models/Ideal_models/Walker_2D_model_SAC')
    ...
    # choose the algorithm for running simulations
    ...
    if not heatmap:
        ...
        file = f'Files/Walker2D/heatmap/{alg}_{perturbation}{new_mass}_{new_friction}_{token}_heatmap.csv'
...
if __name__ == '__main__':
 ...
 #   main(render=False, train=True, pm_pert = 1, alg = 'RARL_PPO') # test RARL PPO
    ...
    main(render=False, train=True, pm_pert = 1.0, alg = 'RARL_SAC') # test RARL SAC

    for path, alg in zip(['Walker_models/Ideal-models/Walker_2D_model_SAC', 'Walker_models/Adversarial_models/Walker_2D_Adversarial_SAC_model'], ['SAC', 'RARL_SAC']):
        for pert in [-0.9, -0.7, -0.5, -0.3, -0.1, 0.0,0.2, 0.5, 0.7, 0.9, 1]:
            for frict in [0.0,0.1, 0.4, 0.8, 1.0, 1.3, 1.7, 2.0, 2.2, 2.5]:
                main(render=False, train=False, pm_pert = pert, frict=frict, alg = alg, model_to_load = f'Models/Walker_2D/{path}', heatmap = True) # test RARL_PPO
